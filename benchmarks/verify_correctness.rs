...
//! Compares normal mode vs `default (non-global)` mode to ensure they find the same matches.
...
fn verify_match(pattern: &str, text: &str, expected_match: Option<&str>) {
    ...
    match (match_normal.as_ref(), match_greedy.as_ref(), expected_match) {
        (Some(n), Some(g), Some(_expected)) => {
            ...
            assert!(!normal_text.is_empty(), "Normal match is empty");
            assert!(!greedy_text.is_empty(), "Greedy match is empty");

            // For greedy mode, we just need A valid match, not necessarily the same one
            // (greedy returns first found, normal may return best)
        }
        (None, None, None) => {
            println!("✓ Pattern: {pattern:?} - correctly found no match in {text:?}");
        }
        (Some(n), None, _) => {
            let normal_text = &text[n.start()..n.end()];
            panic!(
                "MISMATCH: Normal found {normal_text:?} but greedy found nothing\n  Pattern: {pattern:?}\n  Text: {text:?}"
            );
        }
        (None, Some(g), _) => {
            let greedy_text = &text[g.start()..g.end()];
            panic!(
                "MISMATCH: Greedy found {greedy_text:?} but normal found nothing\n  Pattern: {pattern:?}\n  Text: {text:?}"
            );
        }
        (Some(_), Some(_), None) => {
            panic!(
        ...
        (None, None, Some(expected)) => {
            panic!(
                "MISSING: Expected {expected:?} but neither mode found a match\n  Pattern: {pattern:?}\n  Text: {text:?}"
            );
        }
    }
}
